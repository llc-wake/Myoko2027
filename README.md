...
| 05 | 交通住宿 | `#travel` | 5.1 住宿基地 / **5.2 抵日首兩日行程 Arrival plan（17–18 Jan 2027）** / 5.3 Tokyo → Myoko 到達交通 / 5.4 酒店設施 / 5.5 區内飲食 |
...
| `p4.py` | §04 山圖（4 張）＋ §05 5.1住宿 / 5.2 抵日首兩日行程 / 5.3 交通 / 5.4 設施 / 5.5 飲食（子標題用 `.sub-h`）|
...
## Arrival plan (added 2026-08-23, section 5.2)

Lawrence’s planned first two days, now stored on the page so it is not lost:

- **17/1 (Sun)** 20:20 land NRT T1 → Keisei Skyliner NRT T1 → Keisei-Ueno → hotel near Keisei-Ueno (**hotel TBC**, Lawrence will confirm).
  There is **no 22:30 Skyliner** on the Sat/holiday table — nearest are 22:20→23:09, 22:40→23:28, last 23:00→23:48.
- **18/1 (Mon)** 06:10 walk to JR Ueno → Hokuriku Shinkansen **Hakutaka 551** 06:34 → Nagano **08:03** (not 08:06) → Shinano Railway Kita-Shinano 09:23 → Myoko-Kogen 10:08 → Lime Resort free shuttle ~10:20, luggage drop.
- Cost per person 17–18/1 ≈ ¥11,670; two people ≈ ¥23,340 (reserved seats).
- Alternative recorded in an accordion: **Kagayaki 501** Ueno 06:22 → Nagano 07:36, connect 07:45 → Myoko-Kogen 08:31 (9-min transfer risk, hotel shuttle only from 09:00).

**Standing daily duty:** re-verify all three rail legs each run (Skyliner Sat/holiday times, Hakutaka 551 stop times, Kita-Shinano 09:23 service) and flag any change. The January 2027 timetable is only published around **mid-Dec 2026**; until then everything on 5.2 is marked as reference only.
